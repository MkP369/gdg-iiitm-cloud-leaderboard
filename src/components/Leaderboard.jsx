    ...
    <div className="leaderboard-container">
      {searchTerm && (
        <div className="search-results-info">
          {data.length > 0 ? (
            <p>Found {data.length} participant{data.length !== 1 ? 's' : ''} matching "{searchTerm}"</p>
          ) : (
          ...
          {data.map((entry) => (
            <tr key={entry.profileURL}>
              ...
              <td className="status-cell">
                <span className={entry.profileStatus === 'All Good' ? 'status-good' : 'status-issue'}>
                  {entry.profileStatus}
                </span>
              </td>
              <td className="redemption-cell">
                <span className={entry.redemptionStatus === 'Yes' ? 'redemption-yes' : 'redemption-no'}>
